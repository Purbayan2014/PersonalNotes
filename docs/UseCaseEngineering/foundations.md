...
71. API Activity without matching interactive login 
72. Sequential Object ID Access Pattern 
73. Broken Function level authorisation indicator
74. WAF Exploit Attempt
75. Web Exploit followed by server process creation 
76. Suspiciosu File to upload web applications
77. Admin Portal Brute Force
78. Business Logic Abuse in Transaction Flow
79. Internal Host communicates with known malicious IP
80. Malicious Domain Resolution 
---
...
# uscase 71 : API activity  without matching interactive login

Category : API, Application and Web Security
Mitre att&ck mapping : T1550 Use Alternate Authentication Material

Hypothesis : A user token performs api actions even though no matching login session creation is observed

Required log sources :
    - api gateway logs
    - identity logs
    - application session logs
      - identifies the application , mailbox, sharpoint site, api endpoint, database, cloud object, file path or business resource touched with api actitivity without matching interactive login
    - timegenerated/eventtime
    - user/account/service-principal
    - host/device/asset
    - sourceip/geo/asn/destip
...
BB API Activity 
BB Login Session correlation 
BB Service Token Exclusion 

Rule Creation Logic

rule High_Fidelity_API_Activity_Without_Interactive_Login
{
  meta:
    author = "SOC Team"
    description = "Detect API activity without a correlated interactive login session"
    severity = "Critical"
    mitre_tactic = "Credential Access"
    mitre_technique = "T1078.004"
  ...
  events:
    ...
    // Building Blocks
    $activity.security_result.summary = "API_ACTIVITY"

    $session.security_result.summary = "LOGIN_SESSION_CORRELATION"

    $service.security_result.summary = "SERVICE_TOKEN_EXCLUSION"

    $user = $api.principal.user.userid
...
![alt text](../assets/alertdesign71..png)
...
# usecase 72 : Sequential Object ID Acess Pattern 

Category : API, Application and Web Security
Mitre att&ck mapping : T1087 Account Discovery; T1190 Exploit Public-Facing Application

Hypothesis : A user enumerates account or customer ids by changing object identifiers in requests 
...
BB Sequential Object ID Pattern
BB Sensitive Objects
BB Normal Pagination Exclusion 

Rule Creation logic

rule High_Fidelity_Sequential_Object_ID_Access
{
  meta:
    author = "SOC Team"
    description = "Detect sequential object ID enumeration against sensitive application objects"
    severity = "Critical"
  ...
  events:
    ...
    re.regex(
        $api.target.url,
        `(?i)(/users/|/orders/|/documents/|/customers/|/records/)`
    )

    // Building Blocks
    $pattern.security_result.summary = "SEQUENTIAL_OBJECT_ID_PATTERN"

    $object.security_result.summary = "SENSITIVE_OBJECT"

    $pagination.security_result.summary = "NORMAL_PAGINATION_EXCLUSION"

    $client = $api.principal.ip
  ...
  outcome:
    ...
    $risk_score =
        ...
        if(#pattern > 0,20,0) +
        if(#object > 0,20,0) +
        if($requests >= 500,20,10) -
        if(#pagination > 0,30,0)
...
![alt text](../assets/alertdesign72.png)
...
# usecase 73 : Broken Function level authorisation indicator

Category : API, Application and Web Security
Mitre att&ck mapping : T1190 Exploit Public-Facing Application; T1098 Account Manipulation

Hypothesis : A normal user succesfully accesses an administrative api endpoint.

Required log sources :
    - api gateway logs
    - application authz logs
      - dentifies the application, mailbox,SharePoint site, API endpoint, database, cloud object, file path or business resource touched during Broken Function Level Authorisation Indicator. 
    - timegenerated/eventtime
    - user/account/service-principal
    - host/device/asset
    - sourceip/geo/asn/destip
...
BB Admin endpoints
BB Non-Admin Users
BB HTTP 200 or Admin API

Rule Creation logic 

rule High_Fidelity_Broken_Function_Level_Authorization
{
  meta:
    author = "SOC Team"
    description = "Detect successful access to privileged API functions by non-administrative users"
    severity = "Critical"
    mitre_tactic = "Privilege Escalation"
  ...
  events:
    ...
    // Building Blocks
    $admin.security_result.summary = "ADMIN_ENDPOINT"

    $nonadmin.security_result.summary = "NON_ADMIN_USER"

    $success.security_result.summary = "HTTP_200_ON_ADMIN_API"
...
![alt text](../assets/alertdesign73.png)
...
• Create and maintain the building blocks used by this rule: BB: Admin Endpoints, BB: Non-Admin Users, BB: HTTP 200
on Admin API.
• Do not tune by broad user or IP exclusion alone. Prefer conditional exclusions using asset, application, time window,
change ticket and expected source.
• Use dynamic baselines per user, host, application or service account where possible. A single global threshold will
...
# usecase 74 : WAF Exploit Attempt Allowed

Category : API, Application and Web Security
Mitre att&ck mapping : T1190 Exploit Public-Facing Application

Hypothesis : WAF detects SQL injection or exploit pattern but the request is allowed and returns http 200
...
BB WAF Signature
BB Action Allowed
BB Internal-facing Apps

Rule Creation logic

rule High_Fidelity_WAF_Exploit_Attempt_Allowed
{
  meta:
    author = "SOC Team"
    description = "Detect exploit attempts allowed by the WAF against Internet-facing applications"
    severity = "Critical"
  ...
  events:
    ...
    // Building Blocks
    $signature.security_result.summary = "WAF_ATTACK_SIGNATURE"

    $allowed.security_result.summary = "ACTION_ALLOWED"

    $internet.security_result.summary = "INTERNET_FACING_APPLICATION"

    $src_ip = $waf.principal.ip
...
![alt text](../assets/alertdesign74.png)
...
# usecase 75 : Web exploit followed by Server Process Creation

Category : API, Application and Web Security
Mitre att&ck mapping : T1190 Exploit Public-Facing Application; T1059 Command and Scripting Interpreter

Hypothesis : A suspicous web request is followed by command shell execution on the web server
...
BB Exploit Attempt
BB Server Process Creation 
BB External Callback

Rule Creation logic 

rule High_Fidelity_Web_Exploit_Server_Process_Creation
{
  meta:
    author = "SOC Team"
    description = "Detect successful web exploitation resulting in server-side process execution"
    severity = "Critical"
  ...
  events:
    ...
    // External Callback
    $callback.metadata.event_type = "NETWORK_CONNECTION"

    // Building Blocks
    $exploit.security_result.summary = "EXPLOIT_ATTEMPT"

    $process.security_result.summary = "SERVER_PROCESS_CREATION"

    $external.security_result.summary = "EXTERNAL_CALLBACK"

    $host = $proc.principal.hostname

    $proc.principal.hostname = $waf.target.hostname

    $callback.principal.hostname = $host
...
![alt text](../assets/alertdesign75.png)
...
# usecase 76 : Suspicious File Upload to Web applications

Category : API, Application and Web Security
Mitre att&ck mapping : T1190 Exploit Public-Facing Application; T1505.003 Web Shell

Hypothesis : A user uploads a file with script-like content or extension to a web-accesible folder
...
BB Upload Endpoint
BB Executable Extension 
BB Web Directory Write

Rule Creation logic 

rule High_Fidelity_Suspicious_File_Upload
{
  meta:
    author = "SOC Team"
    description = "Detect suspicious uploads followed by file creation on the web server"
    severity = "Critical"
    mitre_tactic = "Initial Access"
    mitre_technique = "T1190"
  ...
  events:
    ...
    // Building Blocks
    $endpoint.security_result.summary = "UPLOAD_ENDPOINT"

    $extension.security_result.summary = "EXECUTABLE_EXTENSION"

    $webdir.security_result.summary = "WEB_DIRECTORY_WRITE"

    $host = $upload.target.hostname
...
![alt text](../assets/alertdesign76.png)
...
# usecase 77 : Admin Portal Brute Force

Category :API, Application and Web Security
Mitre att&ck mapping : T1110 Brute Force; T1190 Exploit Public-Facing Application

hypothesis : A public admin portal receives repeated failed login attempts from one or more sources
...
BB Admin Portal URLs
BB Failed Login Threshold
BB External Source IP

Rule Creation logic

rule High_Fidelity_Admin_Portal_Brute_Force
{
  meta:
    author = "SOC Team"
    description = "Detect repeated authentication failures against admin portals from external sources"
    severity = "Critical"
    mitre_tactic = "Credential Access"
  ...
  events:
    ...
    // Building Blocks
    $admin.security_result.summary = "ADMIN_PORTAL_URL"

    $threshold.security_result.summary = "FAILED_LOGIN_THRESHOLD"

    $external.security_result.summary = "EXTERNAL_SOURCE_IP"
  ...
  outcome:
    ...
    $failed_logins = count($login)

    $risk_score =
        40 +
        if(#admin > 0,20,0) +
        if(#external > 0,20,0) +
        if($failed_logins >= 50,20,10)
...
![alt text](../assets/alertdesign77.png)
...
# usecase 78 : Business Logic Abuse in Transcation Flow 

Category : API, Application and Web Security
Mitre att&ck mapping : T1565 Data Manipulation; T1530 Data from Cloud Storage

Hypothesis : 
A user repeatedly performs small transcations or state changes that avoid normal thresholds but slow abuse pattern
...
BB Sensitive Business Flow
BB Transaction Baseline
BB Fraud Rule Hit

Rule Creation logic 

rule High_Fidelity_Business_Logic_Abuse
{
  meta:
    author = "SOC Team"
    description = "Detect business logic abuse using fraud intelligence and transaction baselines"
    severity = "Critical"
    mitre_tactic = "Impact"
  ...
  events:
    ...
    $txn.metadata.event_type = "APPLICATION_LOG"

    // Building Blocks
    $flow.security_result.summary = "SENSITIVE_BUSINESS_FLOW"

    $baseline.security_result.summary = "TRANSACTION_BASELINE"

    $fraud.security_result.summary = "FRAUD_RULE_HIT"

    $user = $txn.principal.user.userid
...
![alt text](../assets/alertdesign78.png)
...
# usecase 79 : Internal Host Communicates with known malicious IP

Category : Network, Perimeter and Threat Intelligence
Mitre att&ck mapping : T1071 Application Layer Protocol

Hypothesis : An internal host initiate outbount communication to an IP Address present in threat intelligence.
...
BB Threat Intel IP Match 
BB Internal Asset Criticality 
BB Allowed Business Partners

Rule creation logic

rule High_Fidelity_Malicious_IP_Communication
{
  meta:
    author = "SOC Team"
    description = "Detect communication with known malicious infrastructure using threat intelligence correlation"
    severity = "Critical"
  ...
  events:
    ...
    // Building Blocks
    $ti.security_result.summary = "THREAT_INTEL_IP_MATCH"

    $critical.security_result.summary = "INTERNAL_ASSET_CRITICALITY"

    $partner.security_result.summary = "ALLOWED_BUSINESS_PARTNERS"

    $host = $conn.principal.hostname
...
![alt text](../assets/alertdesign79.png)
...
# usecase 80 : Malicious Domain Resolution 

category : Network, Perimeter and Threat Intelligence
Mitre att&ck mapping : T1071.004 DNS

Hypothesis : A workstation resolves to a domain known to be associated with malware or credential phishing 
...
BB Threat Intel Domain Match 
BB Newly Registered Domain 
BB Internal Host 


Rule Creation logic 

rule Enterprise_Malicious_Domain_Resolution
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $ti.security_result.summary = "THREAT_INTEL_DOMAIN_MATCH"

    $nrd.security_result.summary = "NEWLY_REGISTERED_DOMAIN"
  ...
  condition:
    ...
    not $dns.network.dns.questions.name in
        %approved_partner_domains

    and

    not $host in
        %approved_security_scanners
}
...
![alt text](../assets/alertdesign80.png)
...
# uscase 81 : C2 Beaconing Pattern 

Category : Network, Perimeter and Threat Intelligence

Mitre att&ck pattern : T1071 Application Layer Protocol; T1102 Web Service

Hypothesis : A host contacts the same rare destination every 60 seconds within similar byte size
...
Required log sources :  
    ...
    - dns logs
    - ndr 
        - provides supporting evidence for c2 beaconing pattern.
    - timegenerated/eventtime
    - user/account/service-principal
    - host/device/asset
    - sourceip/geo/asn/destip
...
BB Beacon Interval 
BB Rare Destination 
BB Suspicios User Agent 

Rule Creation logic 

rule Enterprise_C2_Beaconing
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $interval.security_result.summary = "BEACON_INTERVAL"

    $rare.security_result.summary = "RARE_DESTINATION"
  ...
  condition:
      ...
      not $dst in
          %approved_partner_ips

      and

      not $host in
          %approved_monitoring_servers
}

Alert Design 

Field Example Value
Alert Name 2026 - C2 Beaconing Pattern
Incident Name Possible C2 Beaconing Pattern Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
Business Impact Tag Network, Perimeter and Threat Intelligence
The alert description should explain the detection in plain language. Analysts should be able to see the user, host, sour
...
# usecase 82 : Internal Port Scanning from workstation

Category : Network, perimeter and Threat intelligence
Mitre att&ck mapping : T1064 Network service discovery 

Hypothesis : A normal workstation scans multiple ports across a server subnet 
...
BB Internal Scan Threshold 
BB User workstations
BB Approved Scanner Hosts 
...
# usecase 83 : VPN Followed by Internal Scan 

category : Network, permiter, threat intelligence
mitre att&ck mapping : T1133 External Remote Services; T1046 Network Service Discovery

Hypothesis : A new-country vpn is followed by internal probing against many hosts 
...
rule Enterprise_VPN_Login_Followed_By_Internal_Scan
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $vpn_session.security_result.summary = "VPN_SESSION"

    $internal_scan.security_result.summary = "INTERNAL_SCAN"

    $new_country.security_result.summary = "NEW_COUNTRY"

    $user = $vpn.principal.user.userid
  ...
  condition:
      ...
      not $vpn.principal.ip in
          %approved_vpn_gateways

      and

      not $user in
          %approved_penetration_test_accounts
}

Alert Design

Field Example Value
Alert Name 2026 - VPN Login Followed by Internal Scan
Incident Name Possible VPN Login Followed by Internal Scan Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 84 : Large outbound transfer to rare destination 

category : Network, Perimeter and Threat Intelligence
mitre att&ck mapping : T1041 Exfiltration Over C2 Channel; T1048 Exfiltration Over Alternative Protocol. 

Hypothesis : An endpoint uploads many gigabytes to a previously unseen external destination. 
...
Required log sources : 
    ...
    - dlp
    - netflow 
        - provides network movement and permiter evidence for large outbound transfer to rare destination 
    - timegenerated/eventtime
    - user/account/service-prinicpal
    - host/device/asset 
    - action/result/status
...
rule Enterprise_Large_Outbound_Transfer
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $upload.security_result.summary = "LARGE_UPLOAD"

    $rare.security_result.summary = "RARE_DESTINATION"

    $asset.security_result.summary = "SENSITIVE_ASSET"
  ...
  outcome:
    ...
    $bytes_sent = sum($conn.network.sent_bytes)

    $risk_score =
        if($bytes_sent >= 5368709120,100,90)   // 5 GB
...
Field Example Value
Alert Name 2026 - Large Outbound Transfer to Rare Destination
Incident Name Possible Large Outbound Transfer to Rare Destination Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 85 : DNS Tunnelling indicator 

category : Network, Permimeter and Threat Intelligence
Mitre att&ck mapping : T1048 Exfiltration Over Alternative Protocol; T1071.004 DNS

Hypothesis : A host generated many long, encoded-looking dns queries to the same domain. 
...
rule Enterprise_DNS_Tunnelling
{
  meta:
    severity = "Critical"
    attack_technique = "T1071.004"
  ...
  events:
    ...
    $long.security_result.summary = "LONG_SUBDOMAIN"

    $entropy.security_result.summary = "HIGH_ENTROPY_DNS"

    $volume.security_result.summary = "QUERY_VOLUME"

    $host = $dns.principal.hostname

    $domain = $dns.network.dns.questions.name
...
Field Example Value
Alert Name 2026 - DNS Tunnelling Indicator
Incident Name Possible DNS Tunnelling Indicator Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 86 : High-Severity IPS Block Against Critical Asset 

Category : Network, Perimeter and Threat Intelligence
Mitre att&ck mapping : T1190 Exploit Public-Facing Application; T1595 Active Scanning

Hypothesis : IPS Block exploit attempt targetting a critical exposed system 
...
Required log sources : 
    ...
    - user/account/service-prinicpal
    - host/device/asset 
    - sourceip/geo/asn/destinationip
    - action/result/status
    - application/resource/object

Building Blocks Design 

BB High Severity IPS
BB Critical Asset 
BB Internet-facing Systems 

Rule Creation logic 

rule Enterprise_High_Severity_IPS_Block
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $ips.metadata.event_type = "NETWORK_INTRUSION_DETECTION"

    $high.security_result.summary = "HIGH_SEVERITY_IPS"

    $critical.security_result.summary = "CRITICAL_ASSET"

    $internet.security_result.summary = "INTERNET_FACING_SYSTEM"

    $src = $ips.principal.ip
  ...
  condition:
      ...
      not $src in
          %approved_penetration_test_ips
}

Alert Design 

Field Example Value
Alert Name 2026 - High-Severity IPS Block Against Critical Asset
Incident Name Possible High-Severity IPS Block Against Critical Asset Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 87 : Sensitive File upload to external storage 

Category : Data Leakage, Insider Risk and AI Security
Mitre att&ck mapping : T1048 Exfiltration Over Alternative Protocol; T1537 Transfer Data to Cloud Account

Hypothesis : A user uploads a file containing customer or confidential data to personal cloud storage 
...
BB Senstive Data Pattern 
BB External Storage Domain 
BB Corporate Approved Cloud 

Rule Creation logic 

rule Enterprise_Sensitive_File_Upload
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $dlp.security_result.summary = "SENSITIVE_DATA_PATTERN"

    $storage.security_result.summary = "EXTERNAL_STORAGE_DOMAIN"

    $approved.security_result.summary = "CORPORATE_APPROVED_CLOUD"

    $user = $upload.principal.user.userid

    $host = $upload.principal.hostname
...
Field Example Value
Alert Name 2026 - Sensitive File Uploaded to External Storage
Incident Name Possible Sensitive File Uploaded to External Storage Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
Business Impact Tag Data Leakage, Insider Risk and AI Security
...
• Escalate according to the incident classification and business impact, not only the technical event type.
• Document the final decision, containment actions, evidence and closure rationale in the case record.
• Identify exactly what data was accessed or transferred and whether regulatory, privacy, customer or management
notification review is required.
• Preserve DLP evidence, file access logs, destination details and user confirmation.
...
# usecase 88 : Sensitive Data Sent to Unapproved Gen-AI Tool

Category : Data Leakage, Insider Risk and AI Security
Mitre att&ck mapping : T1048 Exfiltration Over Alternative Protocol

Hypothesis : An employee pastes or uploads regulated data to public AI chatbot from a corporate device 
...
BB Unapproved AI Domains
BB Sensitive Data Pattern 
BB User Department Risk 

Rule Creation logic 

rule Enterprise_Unapproved_GenAI_Data_Transfer
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $genai.security_result.summary = "UNAPPROVED_AI_DOMAIN"

    $dlp.security_result.summary = "SENSITIVE_DATA_PATTERN"

    $dept.security_result.summary = "USER_DEPARTMENT_RISK"

    $user = $proxy.principal.user.userid

    $host = $proxy.principal.hostname
  ...
  condition:
      ...
      not $proxy.target.hostname in
          %approved_genai_domains
...
Field Example Value
Alert Name 2026 - Sensitive Data Sent to Unapproved GenAI Tool
Incident Name Possible Sensitive Data Sent to Unapproved GenAI Tool Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 89 : Source Code Uploaded to AI Platform

Category : Data Leakage, Insider Risk and AI Security
Mitre att&ck mapping : T1048 Exfiltration Over Alternative Protocol

Hypotheis : A developer submits application source code to an unapproved AI service for troubleshooting

Required log sources : 
    - dlp 
    - proxy logs 
    - developer endpoint logs
    - timegenerated/eventtime
    - user/account/service-prinicpal
    - host/device/asset 
    - sourceip/destip/geo/asn
...
BB Source Code Pattern 
BB AI Domains 
BB Developer Groups 

Rule Creation logic 

rule Enterprise_Source_Code_Upload_To_AI
{
  meta:
    severity = "Critical"
    attack_technique = "T1567"
  ...
  events:
    ...
    $code.security_result.summary = "SOURCE_CODE_PATTERN"

    $ai.security_result.summary = "AI_DOMAINS"

    $developer.security_result.summary = "DEVELOPER_GROUP"
...
Field Example Value
Alert Name 2026 - Source Code Uploaded to AI Platform
Incident Name Possible Source Code Uploaded to AI Platform Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 90 : Mass sharepoint downoad by user 
...
A user downloads thosands of files from a sensitive site within a short time 
...
BB Download Volume Baseline 
BB Sensitive Sites 
BB Risky Sign in 

Rule Creation logic 

rule Enterprise_Mass_SharePoint_Download
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $download.metadata.event_type = "FILE_DOWNLOAD"

    $baseline.security_result.summary = "DOWNLOAD_VOLUME_BASELINE"

    $site.security_result.summary = "SENSITIVE_SITES"

    $signin.security_result.summary = "RISKY_SIGN_IN"

    $user = $download.principal.user.userid

    $host = $download.principal.hostname
  ...
  condition:
      ...
      not $user in
          %approved_bulk_download_users

      and

      not $host in
          %approved_migration_hosts
}

Alert Design

Field Example Value
Alert Name 2026 - Mass SharePoint Download by User
Incident Name Possible Mass SharePoint Download by User Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# uscase 91 : Sensitive Data Copied to USB 

category : Data Leakage, Insider Risk and AI Security
mitre att&ck mapping : T1052 Exfiltration Over Physical Medium

Hypothesis : A user copies confidential documents to removeable media. 

Required log sources : 
    - edr device control 
    - windows removable storage logs 
    - dlp 
    - timegenerated/eventtime 
    - user/account/service-principal
    - host/device/asset 
...
BB Removal media 
BB Sensitive File Copy 
BB Approved USB Devices 

Rule Creation logic 

rule Enterprise_Sensitive_Data_To_USB
{
  meta:
    severity = "Critical"
    attack_technique = "T1052.001"

  events:

    $copy.metadata.event_type = "FILE_COPY"

    $usb.security_result.summary = "REMOVABLE_MEDIA_INSERTED"

    $dlp.security_result.summary = "SENSITIVE_FILE_COPY"

    $approved.security_result.summary = "APPROVED_USB_DEVICE"

    $user = $copy.principal.user.userid

    $host = $copy.principal.hostname
...
Field Example Value
Alert Name 2026 - Sensitive Data Copied to USB
Incident Name Possible Sensitive Data Copied to USB Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 92 : Data Access Spike before user exit 

Category : Data Leakage, Insider Risk and AI Security
Mitre att&ck mapping : T1005 Data from Local System; T1039 Data from Network Shared Drive

Hypothesis : A resigining employeee accessses and downloads far more documents than normal 
...
BB Resigned Users
BB Sensitive Repositories 
BB Baseline Access Volume 

Rule Creation logic 

rule Enterprise_Data_Access_Before_User_Exit
{
  meta:
    severity = "Critical"
    attack_technique = "T1213"
  ...
  events:
    ...
    $access.metadata.event_type = "FILE_READ"

    $hr.security_result.summary = "RESIGNED_USERS"

    $repo.security_result.summary = "SENSITIVE_REPOSITORIES"

    $baseline.security_result.summary = "BASELINE_ACCESS_VOLUME"

    $user = $access.principal.user.userid

    $host = $access.principal.hostname
...
Field Example Value
Alert Name 2026 - Data Access Spike Before User Exit
Incident Name Possible Data Access Spike Before User Exit Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
notification review is required.
• Preserve DLP evidence, file access logs, destination details and user confirmation.
11. False Positive Possibilities
• Approved maintenance, migration, testing, vulnerability assessment or administrative activity.
• Known automation account, service integration, backup process, security tool, scanner or deployment platform.
• Business travel, remote work, vendor support activity or approved temporary access.
• False reputation match, geolocation inaccuracy, NAT/proxy aggregation or log parsing issue.
...
# usecase 93 : Priviledged Users Export Sensitive Report 

Category : Data Leakage, Insider Risk and AI Security
Mitre att&ck mapping : T1005 Data from Local System; T1530 Data from Cloud Storage

Hypothesis : An administrator exports a full customer or employee report outside expected operational need. 
...
BB Priviledged Users
BB Sensitive Export Functions 
BB Approved Report Requests 

Rule Creation logic 

rule Enterprise_Privileged_User_Report_Export
{
  meta:
    severity = "Critical"
    attack_technique = "T1213"
  ...
  events:
    ...
    $export.metadata.event_type = "APPLICATION_ACTIVITY"

    $priv.security_result.summary = "PRIVILEGED_USERS"

    $report.security_result.summary = "SENSITIVE_EXPORT_FUNCTIONS"

    $approved.security_result.summary = "APPROVED_REPORT_REQUESTS"

    $user = $export.principal.user.userid

    $host = $export.principal.hostname
  ...
  condition:
      ...
      not $user in
          %approved_report_export_users

      and

      not $host in
          %approved_reporting_servers
}

Alert Design
Field Example Value
Alert Name 2026 - Privileged User Exports Sensitive Report
Incident Name Possible Privileged User Exports Sensitive Report Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 94 : Email with sensitive attachment sent to personal domain 

category : Data Leakage, Insider Risk and AI Security
mitre att&ck mapping : Data Leakage, Insider Risk and AI Security

Hypothesis : A user sneds a spreasheet containing confidential information to a personal email address

Required log sources : 
    - email dlp 
    - exchange message tactics 
    - m365 audit 
    - timegenerated/eventtime 
    - user/account/service-prinicpal
    - host/device/asset 
...
BB Personal Email Domains 
BB Sensitive Attachment Pattern 
BB Approved Exceptions

Rule Creation logic 

rule Enterprise_Sensitive_Email_To_Personal_Domain
{
  meta:
    severity = "Critical"
    attack_technique = "T1567"
  ...
  events:
    ...
    $email.metadata.event_type = "EMAIL_SENT"

    $personal.security_result.summary = "PERSONAL_EMAIL_DOMAINS"

    $attachment.security_result.summary = "SENSITIVE_ATTACHMENT_PATTERN"

    $approved.security_result.summary = "APPROVED_EXCEPTIONS"

    $user = $email.principal.user.userid

    $host = $email.principal.hostname
  ...
  condition:
      ...
      not $user in
          %approved_external_email_users

      and

      not $email.target.user.email_addresses in
...
Field Example Value
Alert Name 2026 - Email with Sensitive Attachment Sent to Personal Domain
Incident Name Possible Email with Sensitive Attachment Sent to Personal Domain Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 95 : Repeated Access Denied to Restricted Folders 

Category : Data Leakage, Insider Risk and AI Security
Mitre att&ck mapping : T1083 File and Directory Discovery; T1039 Data from Network Shared Drive

Hypothesis : A user repeatedly attempts to open folders outside their job role 
...
BB Restricted Folder Paths 
BB Access Denied 
BB Department Role Mapping 

Rule Creation logic 

rule Enterprise_Repeated_Access_Denied_Restricted_Folders
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $folder.security_result.summary = "RESTRICTED_FOLDER_PATHS"

    $threshold.security_result.summary = "ACCESS_DENIED_THRESHOLD"

    $role.security_result.summary = "DEPARTMENT_ROLE_MAPPING"
  ...
  condition:
      ...
      not $host in
          %approved_scanner_hosts
}

Alert Design 

Field Example Value
Alert Name 2026 - Repeated Access Denied to Restricted Folders
Incident Name Possible Repeated Access Denied to Restricted Folders Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 96 : Abnormal Sensitive Printing Volume 

Category : Data Leakage, Insider Risk and AI Security
Mitre att&ck mapping : T1052 Exfiltration Over Physical Medium

Hypothesis : A user prints hundreds of pages of sensititve documents outside normal usage 
...
BB Sensitive Print Jobs 
BB Print Volume Baseline 
BB Restricted Printers 

Rule Creation logic 

rule Enterprise_Abnormal_Sensitive_Printing
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $print.metadata.event_type = "PRINT_JOB"

    $sensitive.security_result.summary = "SENSITIVE_PRINT_JOBS"

    $baseline.security_result.summary = "PRINT_VOLUME_BASELINE"

    $restricted.security_result.summary = "RESTRICTED_PRINTERS"

    $user = $print.principal.user.userid

    $host = $print.principal.hostname
  ...
  condition:
      ...
      not $user in %approved_bulk_print_users

      and

      not $host in %approved_print_servers

      and

      not $print.target.resource.name in %approved_printers
}

Alert Design 

Field Example Value
Alert Name 2026 - Abnormal Sensitive Printing Volume
Incident Name Possible Abnormal Sensitive Printing Volume Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 97 : Critical log source stops sending logs to siem 

Category : Governance, Control Monitoring and Crypto Readiness
Mitre att&ck mapping : T1562 Impair Defenses

Hypothesis : A firewall, edr, identity or core application log sources goes silent, reducing SOC visibility.
...
BB Critical Log sources 
BB No log threshold 
BB Maintenance window

Rule Creation logic 

rule Enterprise_Critical_Log_Source_Offline
{
  meta:
    severity = "Critical"
    attack_technique = "T1562.008"
  ...
  events:
    ...
    $health.metadata.event_type = "SIEM_HEALTH"

    $critical.security_result.summary = "CRITICAL_LOG_SOURCES"

    $threshold.security_result.summary = "NO_LOG_THRESHOLD"

    $maintenance.security_result.summary = "MAINTENANCE_WINDOW"

    $asset = $health.principal.asset.asset_id

    $hostname = $health.principal.hostname
  ...
  condition:
      ...
      not $asset in %approved_maintenance_assets

      and

      not $hostname in %approved_log_collectors
}

Alert Design 

Field Example Value
Alert Name 2026 - Critical Log Source Stops Sending Logs to SIEM
Incident Name Possible Critical Log Source Stops Sending Logs to SIEM Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
Business Impact Tag Governance, Control Monitoring and Crypto Readiness
...
# usecase 98 : SIEM Detection rule disabled or modified 

Category : Governance, Control Monitoring and Crypto Readiness
mitre att&ck mapping : T1562 Impair Defenses

Hypothesis : A high-value detection rule is disabled or its threshold is weakened
...
BB Critical Detection rules
BB Approved rule Change 
BB SOC Admin Accounts 

Rule Creation logic 

rule Enterprise_SIEM_Detection_Rule_Disabled
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $rule.metadata.event_type = "SIEM_AUDIT"

    $critical.security_result.summary = "CRITICAL_DETECTION_RULES"

    $approved.security_result.summary = "APPROVED_RULE_CHANGE"

    $admin.security_result.summary = "SOC_ADMIN_ACCOUNTS"

    $user = $rule.principal.user.userid

    $host = $rule.principal.hostname
  ...
  condition:
      ...
      not $user in %approved_soc_admins

      and

      not $host in %approved_siem_management_hosts
}

Alert Design 

Field Example Value
Alert Name 2026 - SIEM Detection Rule Disabled or Modified
Incident Name Possible SIEM Detection Rule Disabled or Modified Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 99 : Private Key or Certificate Exported unexpectedly 

Category : Governance, Control Monitoring and Crypto Readiness
Mitre att&ck mapping : T1552 Unsecured Credentials

Hypothesis : A private certificate key is exported from a sensitive system outside an approved PKI operation 
...
BB Private Key Paths 
BB Certificate Export Events 
BB Approved PKI Admins 

Rule Creation logic 

rule Enterprise_Private_Key_Certificate_Export
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $export.metadata.event_type = "CERTIFICATE_EXPORT"

    $key.security_result.summary = "PRIVATE_KEY_PATHS"

    $cert.security_result.summary = "CERTIFICATE_EXPORT_EVENTS"

    $admin.security_result.summary = "APPROVED_PKI_ADMINS"
  ...
  condition:
      ...
      not $user in %approved_pki_admins

      and

      not $host in %approved_certificate_servers
}
...
Field Example Value
Alert Name 2026 - Private Key or Certificate Exported Unexpectedly
Incident Name Possible Private Key or Certificate Exported Unexpectedly Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
...
# usecase 100 : Quantam-vulnerable Cryptography on Sensitive Long-live data 

Category : Governance, Control Monitoring and Crypto Readiness
Mitre att&ck mapping: T1552 Unsecured Credentials; Governance Control

Hypothesis : A system storing long-retention sensitive data relies on legacy Cryptography and has nocrypto-agility roadmap 
...
BB Sensitive Long-live data 
BB RSA/ECC inventory
BB Critical Applications 

Rule Creation logic 

rule Enterprise_Quantum_Vulnerable_Cryptography
{
  meta:
    severity = "Critical"
  ...
  events:
    ...
    $crypto.metadata.event_type = "CRYPTOGRAPHY_INVENTORY"

    $data.security_result.summary = "SENSITIVE_LONG_LIVED_DATA"

    $algorithm.security_result.summary = "RSA_ECC_INVENTORY"

    $critical.security_result.summary = "CRITICAL_APPLICATIONS"

    $asset = $crypto.principal.asset.asset_id

    $host = $crypto.principal.hostname
  ...
  condition:
      ...
      not $asset in %approved_legacy_crypto_assets

      and

      not $host in %approved_crypto_exception_hosts
}

Alert Design
Field Example Value
Alert Name 2026 - Quantum-Vulnerable Cryptography on Sensitive Long-Lived Data
Incident Name Possible Quantum-Vulnerable Cryptography on Sensitive Long-Lived Data Investigation
Primary Entity User, Host, Source IP, Application, Session ID
Supporting Context GeoIP, ASN, user privilege, device compliance, asset criticality, data sensitivity, threat-
intel match, related change ticket
Grouping Key Same user/host/source IP/session/application within 24 hours
